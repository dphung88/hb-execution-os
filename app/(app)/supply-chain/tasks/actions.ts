...
export async function createScTaskAction(formData: FormData) {
  let client;
  try { client = createWriteClient(); }
  catch { redirect("/supply-chain/tasks?error=save-not-configured"); }
  ...
  const { error } = await client.from("sc_tasks").insert(payload);
  if (error) redirect(`/supply-chain/tasks?error=${getErrorKey(error.message)}`);

  revalidatePath("/supply-chain");
  revalidatePath("/supply-chain/tasks");
  redirect("/supply-chain/tasks?saved=create");
}

export async function updateScTaskAction(formData: FormData) {
  ...
  revalidatePath("/supply-chain");
  revalidatePath("/supply-chain/tasks");
  redirect(`/supply-chain/tasks?saved=${taskId}`);
}
