...
const sections: Section[] = [
  { label: "Regulatory", rows: ["Submissions Filed", "Approvals Received", "Rejections", "Pending Decisions", "Avg Review Time (days)"] },
  { label: "Quality", rows: ["NCRs Opened", "NCRs Closed", "CAPA Completed", "Audit Findings", "Compliance Rate %"] },
  { label: "Clinical", rows: ["Active Trials", "Patients Enrolled", "Site Visits Completed", "Protocol Deviations", "SAEs Reported"] },
  { label: "Pharmacovigilance", rows: ["Adverse Events Received", "Cases Submitted to Authority", "Avg Case Closure (days)"] },
];

export default async function MedicalResultsPage({ searchParams }: Props) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-white/70 bg-slate-950 px-6 py-8 text-white shadow-panel">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-4xl">Medical Results — {periodLabel}</h1>
            <p className="mt-3 text-sm text-slate-400">Regulatory, quality, clinical, and pharmacovigilance summary.</p>
          </div>
      ...
      <section className="rounded-3xl border border-white/70 bg-white/85 p-6 shadow-panel">
        ...
        <p className="mt-1 text-sm text-slate-500">Medical results integration pending — structure ready for data.</p>
