...
export async function createMedicalTaskAction(formData: FormData) {
  let client;
  try { client = createWriteClient(); }
  catch { redirect("/medical/tasks?error=save-not-configured"); }
  ...
  const { error } = await client.from("medical_tasks").insert(payload);
  if (error) redirect(`/medical/tasks?error=${getErrorKey(error.message)}`);

  revalidatePath("/medical");
  revalidatePath("/medical/tasks");
  redirect("/medical/tasks?saved=create");
}

export async function updateMedicalTaskAction(formData: FormData) {
  ...
  revalidatePath("/medical");
  revalidatePath("/medical/tasks");
  redirect(`/medical/tasks?saved=${taskId}`);
}
