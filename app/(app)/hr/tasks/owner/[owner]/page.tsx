...
import { HR_STATUSES } from "@/lib/hr/config";
import { updateHrTaskAction } from "@/app/(app)/hr/tasks/actions";
...
export default async function HrOwnerDetailPage({ params, searchParams }: Props) {
  ...
  const { tasks: allTasks } = await loadHrTasks(selectedPeriod);
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-[2rem] border border-white/70 bg-slate-950 px-6 py-8 text-white shadow-panel">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-[11px] font-medium uppercase tracking-[0.24em] text-sky-300">HR Tasks · Owner Detail</p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link href={`/hr/tasks?period=${selectedPeriod}`}
              ...
              <ArrowLeft className="h-4 w-4" /> HR Tasks
            </Link>
          </div>
        </div>
      </section>
          ...
          <div className="mt-5 space-y-4">
            {sorted.map((task) => (
              <div key={task.id} className={`rounded-2xl border p-5 ${toneClass(task.status)}`}>
                ...
                <form action={updateHrTaskAction} className="mt-4 flex flex-wrap items-end gap-3 border-t border-current/10 pt-4">
                  <input type="hidden" name="task_id" value={task.id} />
                  <input type="hidden" name="file_link" value={task.fileLink ?? ""} />
                  <input type="hidden" name="redirect_to" value={`/hr/tasks/owner/${encodeURIComponent(owner)}`} />
