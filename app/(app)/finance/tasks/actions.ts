...
export async function createFinanceTaskAction(formData: FormData) {
  ...
  catch { redirect("/finance/tasks?error=save-not-configured"); }
  ...
  const { error } = await client.from("finance_tasks").insert(payload);
  if (error) redirect(`/finance/tasks?error=${getErrorKey(error.message)}`);

  revalidatePath("/finance");
  revalidatePath("/finance/tasks");
  redirect("/finance/tasks?saved=create");
}
...
export async function updateFinanceTaskAction(formData: FormData) {
  ...
  revalidatePath("/finance");
  revalidatePath("/finance/tasks");
  redirect(`/finance/tasks?saved=${taskId}`);
}
