...
export async function createSalesTaskAction(formData: FormData) {
  ...
  catch { redirect("/sales-performance/tasks?error=save-not-configured"); }
  ...
  const { error } = await client.from("sales_tasks").insert(payload);
  if (error) redirect(`/sales-performance/tasks?error=${getErrorKey(error.message)}`);

  revalidatePath("/sales-performance");
  revalidatePath("/sales-performance/tasks");
  redirect("/sales-performance/tasks?saved=create");
}

export async function updateSalesTaskAction(formData: FormData) {
  ...
  revalidatePath("/sales-performance");
  revalidatePath("/sales-performance/tasks");
  redirect(`/sales-performance/tasks?saved=${taskId}`);
}

export async function deleteSalesTaskAction(formData: FormData) {
  const taskId = String(formData.get("task_id") ?? "");
  let client;
  try { client = createWriteClient(); }
  catch { redirect("/sales-performance/tasks?error=save-not-configured"); }

  const { error } = await client.from("sales_tasks").delete().eq("id", taskId);
  if (error) redirect(`/sales-performance/tasks?error=${getErrorKey(error.message)}`);

  revalidatePath("/sales-performance");
  revalidatePath("/sales-performance/tasks");
  redirect("/sales-performance/tasks?saved=delete");
}
