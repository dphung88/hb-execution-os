...
export async function createItTaskAction(formData: FormData) {
  let client;
  try { client = createWriteClient(); }
  catch { redirect("/it/tasks?error=save-not-configured"); }
  ...
  const { error } = await client.from("it_tasks").insert(payload);
  if (error) redirect(`/it/tasks?error=${getErrorKey(error.message)}`);

  revalidatePath("/it");
  revalidatePath("/it/tasks");
  redirect("/it/tasks?saved=create");
}

export async function updateItTaskAction(formData: FormData) {
