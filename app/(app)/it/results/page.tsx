...
const sections: Section[] = [
  { label: "Helpdesk", rows: ["Tickets Opened", "Tickets Closed", "Avg Resolution Time (hrs)", "First Response < 1hr %", "User Satisfaction Score"] },
  { label: "Infrastructure", rows: ["System Uptime %", "Incidents (P1/P2)", "Planned Maintenance Windows", "Backup Success Rate %"] },
  { label: "Development & Deployments", rows: ["Features Deployed", "Hotfixes Released", "Deployments with Zero Downtime", "Avg Deploy Time (min)"] },
  { label: "Security", rows: ["Security Patches Applied", "Vulnerabilities Resolved", "Failed Login Attempts", "Firewall Rule Updates"] },
];

export default async function ItResultsPage({ searchParams }: Props) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-white/70 bg-slate-950 px-6 py-8 text-white shadow-panel">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-4xl">IT Results — {periodLabel}</h1>
            <p className="mt-3 text-sm text-slate-400">Helpdesk, infrastructure, deployment, and security summary.</p>
          </div>
      ...
      <section className="rounded-3xl border border-white/70 bg-white/85 p-6 shadow-panel">
        <h2 className="text-lg font-semibold text-slate-900">IT Summary</h2>
        <p className="mt-1 text-sm text-slate-500">IT results integration pending — structure ready for data.</p>
