...
export function MedicalTasksWorkspace({ tasks, source, searchParams, periods = [], selectedPeriod = "" }: Props) {
  const ownerFilter  = searchParams?.owner  ?? "all";
  const statusFilter = searchParams?.status ?? "all";
  const savedState   = searchParams?.saved;
  const errorState   = searchParams?.error;

  const allStatuses = Array.from(new Set([...MEDICAL_STATUSES, ...tasks.map((t) => t.status)]));
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-[2rem] border border-white/70 bg-slate-950 px-6 py-8 text-white shadow-panel">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-sky-300">Medical Tasks</p>
            <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-4xl">
              Daily task workspace for the Medical team.
            </h1>
            <p className="mt-4 text-sm leading-7 text-slate-300">
              Track regulatory submissions, quality audits, clinical monitoring, pharmacovigilance, and compliance tasks — all in one place.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            {periods.length > 0 && (
              <PeriodSelector periods={periods} selectedPeriod={selectedPeriod} basePath="/medical/tasks" />
            )}
            <div className="hidden h-6 w-px bg-white/15 sm:block" />
            <Link href="/medical" className="inline-flex h-11 items-center justify-center rounded-2xl border border-white/15 bg-white/8 px-4 text-sm font-semibold text-white transition hover:bg-white/15">
              Medical Dashboard
            </Link>
          </div>
        </div>
      ...
      {savedState && (
        <section className="rounded-3xl border border-emerald-200 bg-emerald-50/90 p-5 shadow-panel">
          ...
          <p className="mt-1 text-sm text-emerald-800">{savedState === "create" ? "New medical task created." : "Task updated successfully."}</p>
        </section>
      )}
      {errorState && (
        <section className="rounded-3xl border border-rose-200 bg-rose-50/90 p-5 shadow-panel">
          ...
          <p className="mt-1 text-sm text-rose-800">
            {errorState === "missing-table" ? "The medical_tasks table is missing — run the SQL migration first."
      ...
      <section className="rounded-3xl border border-white/70 bg-white/85 p-6 shadow-panel">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-2xl font-semibold text-slate-900">Create A Medical Task</h2>
          </div>
        </div>
        <form action={createMedicalTaskAction} className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <label className="block">
            ...
            <select name="owner_name" className="mt-2 h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-900 outline-none transition focus:border-sky-400">
              {MEDICAL_OWNERS.map((o) => <option key={o} value={o}>{o}</option>)}
            </select>
          </label>
          <label className="block">
            <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Requester</span>
            <input name="request_source" placeholder="e.g. CEO, Medical Director" className="mt-2 h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-900 outline-none transition focus:border-sky-400" />
          </label>
          <label className="block">
          ...
          <div className="min-w-[860px]">
            ...
            <div className="divide-y divide-slate-100">
              {boardRows.map(({ owner, task }) => (
                <form key={task?.id ?? owner} action={task ? updateMedicalTaskAction : undefined}
