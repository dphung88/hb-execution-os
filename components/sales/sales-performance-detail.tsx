        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            <div key={card.label} className={`rounded-2xl border p-5 ${card.color}`}>
              <p className={`text-[11px] font-semibold uppercase tracking-[0.18em] ${"dark" in card && card.dark ? "text-slate-400" : "text-slate-500"}`}>{card.label}</p>
              <p className={`mt-3 text-xl font-semibold leading-tight ${"dark" in card && card.dark ? "text-white" : "text-slate-900"}`}>{card.value}</p>
              <p className={`mt-2 text-xs ${"dark" in card && card.dark ? "text-slate-400" : "text-slate-500"}`}>{card.note}</p>
            </div>
          ))}
        </div>
