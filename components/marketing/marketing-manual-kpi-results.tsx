...
export function MarketingManualKpiResults({
  ...
  useEffect(() => {
    ...
    setSelectedRoleId((current) => current || roleResults[0].id);
  }, [roleResults]);

  const selectedRole = roleResults.find((role) => role.id === selectedRoleId) ?? roleResults[0];
    ...
    <>
      <section className="rounded-3xl border border-white/70 bg-white/85 p-6 shadow-panel">
        ...
        <div className="mt-6 space-y-5">
          <div className="rounded-3xl border border-slate-200 bg-slate-50 p-4 md:hidden">
            <div className="flex items-center justify-between gap-3">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Role selector</p>
              <button
                type="button"
              ...
              >
                {selectorOpen ? "Hide list" : "Change role"}
              </button>
            </div>
            ...
            {selectedRole ? (
              <button
                type="button"
                onClick={() => setSelectorOpen((open) => !open)}
                className="mt-3 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-left text-sm font-semibold text-slate-900"
              >
                {selectedRole.role}
              </button>
            ) : null}
          ...
          {selectedRole ? (
            <div key={selectedRole.id} className="rounded-3xl border border-slate-200 bg-slate-50 p-4 md:hidden">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="text-base font-semibold text-slate-900">{selectedRole.role}</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-700">
                    Workbook score {selectedRole.workbookScore}/100
                  </span>
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-700">
                    Task execution {selectedRole.executionScore}/40
                  </span>
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-slate-700">
                    Payout {selectedRole.payoutPercent}% / {formatPlainNumber(selectedRole.payoutAmount)} VND
                  </span>
                </div>
              </div>
                  ...
                  <div key={section.id} className="rounded-2xl border border-slate-200 bg-white p-3.5">
                    ...
                    <div className="mt-3 grid gap-3">
                      {section.metrics.map((metric) => {
                        const key = `${selectedRole.id}:${metric.id}`;
