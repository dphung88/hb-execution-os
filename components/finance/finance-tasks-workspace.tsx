...
export function FinanceTasksWorkspace({ tasks, source, searchParams, periods = [], selectedPeriod = "" }: Props) {
  ...
  const allStatuses = Array.from(new Set([...FINANCE_STATUSES, ...tasks.map((t) => t.status)]));
  ...
  // Board: one row per owner (latest task)
  const boardRows = FINANCE_OWNERS.map((owner) => {
      ...
      <section className="rounded-[2rem] border border-white/70 bg-slate-950 px-6 py-8 text-white shadow-panel">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-sky-300">Finance Tasks</p>
            <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-4xl">
              Daily task workspace for the Finance team.
            </h1>
            <p className="mt-4 text-sm leading-7 text-slate-300">
              Create and track finance deliverables — closing, reporting, reconciliation, and compliance tasks — all in one place.
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            {periods.length > 0 && (
              <PeriodSelector periods={periods} selectedPeriod={selectedPeriod} basePath="/finance/tasks" />
            )}
      ...
      {savedState && (
        <section className="rounded-3xl border border-emerald-200 bg-emerald-50/90 p-5 shadow-panel">
          ...
          <p className="mt-1 text-sm text-emerald-800">
            {savedState === "create" ? "New finance task created." : "Task updated successfully."}
          </p>
        </section>
      )}
      {errorState && (
        <section className="rounded-3xl border border-rose-200 bg-rose-50/90 p-5 shadow-panel">
          ...
          <p className="mt-1 text-sm text-rose-800">
            {errorState === "missing-table"
              ? "The finance_tasks table is missing — run the SQL migration first."
      ...
      <section className="rounded-3xl border border-white/70 bg-white/85 p-6 shadow-panel">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="text-2xl font-semibold text-slate-900">Create A Finance Task</h2>
          </div>
        </div>
        ...
        <form action={createFinanceTaskAction} className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <label className="block">
            ...
            <select name="owner_name" className="mt-2 h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-900 outline-none transition focus:border-sky-400">
              {FINANCE_OWNERS.map((o) => <option key={o} value={o}>{o}</option>)}
            </select>
          </label>
          <label className="block">
            <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500">Requester</span>
            <input name="request_source" placeholder="e.g. CEO, CFO" className="mt-2 h-11 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm text-slate-900 outline-none transition focus:border-sky-400" />
          </label>
          <label className="block">
          ...
          <div className="min-w-[860px]">
            ...
            <div className="divide-y divide-slate-100">
              {boardRows.map(({ owner, task }) => (
                <form
                  key={task?.id ?? owner}
                  action={task ? updateFinanceTaskAction : undefined}
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      ...
                      {task && (
                        <span className={`rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${toneClass(task.status)}`}>
                          {task.status}
                        </span>
                      )}
                    </div>
                  ...
                  <div>
                    <input type="number" name="progress_percent" defaultValue={task?.progressPercent ?? 0}
                      min={0} max={100} disabled={!task}
                      className="h-9 w-full rounded-xl border border-slate-200 bg-white px-2.5 text-xs text-slate-900 outline-none transition focus:border-sky-400" />
                  </div>
