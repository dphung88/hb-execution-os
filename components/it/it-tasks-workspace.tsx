...
import { createItTaskAction } from "@/app/(app)/it/tasks/actions";
...
export function ItTasksWorkspace({ tasks, source, searchParams, periods = [], selectedPeriod = "" }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Workload — full width, horizontal cards, each links to owner detail */}
      <section className="rounded-3xl border border-white/70 bg-white/85 p-6 shadow-panel">
        <div className="flex items-center gap-3">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
          {IT_OWNERS.map((owner) => {
            ...
            return (
              <Link
                key={owner}
                href={`/it/tasks/owner/${encodeURIComponent(owner)}?period=${selectedPeriod}`}
                ...
                {r.total > 0 && (
                  <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-slate-100">
                    <div
                      ...
                      style={{ width: `${Math.round((r.completed / r.total) * 100)}%` }}
                    />
                  </div>
                )}
      ...
      <section className="rounded-3xl border border-white/70 bg-white/85 p-6 shadow-panel">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {overdue.length ? overdue.map((t) => (
            <Link
              key={t.id}
              href={`/it/tasks/owner/${encodeURIComponent(t.owner)}?period=${selectedPeriod}`}
            ...
            >
              ...
              <p className="mt-2 text-[10px] font-semibold uppercase tracking-wide text-rose-500 opacity-0 transition group-hover:opacity-100">
                View & update →
              </p>
            </Link>
          )) : (
