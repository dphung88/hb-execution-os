...
import type { ItTaskRecord } from "@/lib/it/config";

export type { ItTaskRecord } from "@/lib/it/config";
export { IT_OWNERS, IT_STATUSES } from "@/lib/it/config";
...
function fromSheet(t: Awaited<ReturnType<typeof fetchDeptSheetTasks>>["tasks"][number], monthKey: string): ItTaskRecord {
...
export async function loadItTasks(monthKey: string): Promise<ItTasksSource> {
  if (!hasSupabaseClientEnv()) {
    const s = await fetchDeptSheetTasks("IT");
  ...
  try {
    ...
    const { data, error } = await supabase.from("it_tasks")
