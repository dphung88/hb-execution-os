...
import type { SupplyChainTaskRecord } from "@/lib/supply-chain/config";

export type { SupplyChainTaskRecord } from "@/lib/supply-chain/config";
export { SC_OWNERS, SC_STATUSES } from "@/lib/supply-chain/config";
...
function fromSheet(t: Awaited<ReturnType<typeof fetchDeptSheetTasks>>["tasks"][number], monthKey: string): SupplyChainTaskRecord {
...
export async function loadScTasks(monthKey: string): Promise<ScTasksSource> {
  if (!hasSupabaseClientEnv()) {
    const s = await fetchDeptSheetTasks("SC");
  ...
  try {
    ...
    const { data, error } = await supabase.from("sc_tasks")
