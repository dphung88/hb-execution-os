...
import type { HrTaskRecord } from "@/lib/hr/config";

export type { HrTaskRecord } from "@/lib/hr/config";
export { HR_OWNERS, HR_STATUSES } from "@/lib/hr/config";
...
function fromSheet(t: Awaited<ReturnType<typeof fetchDeptSheetTasks>>["tasks"][number], monthKey: string): HrTaskRecord {
...
export async function loadHrTasks(monthKey: string): Promise<HrTasksSource> {
  if (!hasSupabaseClientEnv()) {
    const s = await fetchDeptSheetTasks("HR");
  ...
  try {
    ...
    const { data, error } = await supabase.from("hr_tasks")
