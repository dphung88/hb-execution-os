...
import type { SalesTaskRecord } from "@/lib/sales/config";

export type { SalesTaskRecord } from "@/lib/sales/config";
export { SALES_OWNERS, SALES_STATUSES } from "@/lib/sales/config";
...
function fromSheet(t: Awaited<ReturnType<typeof fetchDeptSheetTasks>>["tasks"][number], monthKey: string): SalesTaskRecord {
...
export async function loadSalesTasks(monthKey: string): Promise<SalesTasksSource> {
  if (!hasSupabaseClientEnv()) {
    const s = await fetchDeptSheetTasks("SALES");
  ...
  try {
    ...
    const { data, error } = await supabase.from("sales_tasks")
